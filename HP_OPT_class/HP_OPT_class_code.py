# The code has been written using the guidlines "https://peps.python.org/pep-0008/"
#  e.g. 4 spaces are used for indentation,
# a blank line will separate classes and functions, and spaces are put around
...
class HPOpt:
    def __init__(self, x_train, y_train, batch_size=None, n_trials=None, n_epochs =None,
        ...
        perception, XGboost, Convolutional neural network (1D),
        and transformers for the classification of tabular data.
        Use case:
        hp_optimizer = HPOpt(x_train, y_train, batch_size=32, n_trials=10, mlp_hyp_par=None,
          xgb_hyp_para=None, cnn_hyp_par=None, transformer_hyp_par=None, num_classes=None)
        study_keras = hp_optimizer.optimize("keras")
        Args:
            x_train             (pd.core.frame.DataFrame)    : Should contain variables for training
            y_train             (pd.core.series.Series)      : The target array
            batch_size          (int)                        : The batch size of the NN
            n_trials            (int)                        : The # of trials for the
                                                               Optuna HP optimization
            n_epochs            (int)                        : The # of epochs/trial of Optuna
            mlp_hyp_par         (dict)                       : A dict of the HP of the MLP
            xgb_hyp_para        (dict)                       : A dict of the HP of the
                                                                XGBoost Classifier
            cnn_hyp_par         (dict)                       : A dict of the HP of the CNN
            transformer_hyp_par (dict)                       : A dict of the HP of the transformer
        """
        ...
        self.xgb_hyp_para = xgb_hyp_para or { 'n_estimators': (100, 1000),
                                              'alpha': (2, 30),
                                              'gamma': (0, 1),
        ...
        self.transformer_hyp_par = transformer_hyp_par or {'d_model': (64, 256),
                                                           'num_heads': (2, 8),
                                                           'num_layers': (2, 15),
                                                           'dropout': (0.1, 0.5),
    ...
    def mlp_objective(self, trial):
        """"
        This method takes the hyperparameters of the MLP,
        creates a model and then trains and tests the model
        """
        le = LabelEncoder()
        ...
        n_layers = trial.suggest_int("n_layers",
                                      self.mlp_hyp_par['n_layers'][0],
                                        self.mlp_hyp_par['n_layers'][1])
        model = Sequential()
        model.add(tf.keras.layers.Flatten(input_shape=(self.X_train.shape[1], )))
        for i in range(n_layers):
            num_hidden = trial.suggest_int("n_units_l{}".format(i),
                                           self.mlp_hyp_par['n_units'][0],
                                           self.mlp_hyp_par['n_units'][1],
                                           log=True)
            model.add(tf.keras.layers.Dense(num_hidden, activation="relu"))
        model.add(tf.keras.layers.Dense(self.num_classes, activation=tf.nn.softmax))
        learning_rate = trial.suggest_float("learning_rate", self.mlp_hyp_par['learning_rate'][0],
                                            self.mlp_hyp_par['learning_rate'][1], log=True)
        model.compile(
    ...
    def xgboost_objective(self, trial):
        """"
        This method takes the hyperparameters of the XGBoost,
        creates a model and then trains and tests the model
        """
        param = {
            'booster': 'gbtree',
            "n_estimators": trial.suggest_int("n_estimators",
                                              self.xgb_hyp_para['n_estimators'][0],
                                              self.xgb_hyp_para['n_estimators'][1],
                                              step=10),
            "alpha": trial.suggest_int("alpha",
                                        self.xgb_hyp_para['alpha'][0],
                                        self.xgb_hyp_para['alpha'][1]),
            "gamma": trial.suggest_float("gamma",
                                         self.xgb_hyp_para['gamma'][0],
                                         self.xgb_hyp_para['gamma'][1]),
            "learning_rate": trial.suggest_float("learning_rate",
                                                 self.xgb_hyp_para['learning_rate'][0],
                                                 self.xgb_hyp_para['learning_rate'][1],
                                                 log=True),
    ...
    def cnn_objective(self, trial):
        """"
        This method takes the hyperparameters of the CNN,
        creates a model and then trains and tests the model
        """
        ...
        model.add(Flatten())
        n_layers = trial.suggest_int("n_layers",
                                     self.cnn_hyp_par['n_layers_min'],
                                     self.cnn_hyp_par['n_layers_max'])
        for i in range(n_layers):
            num_hidden = trial.suggest_int("n_units_l{}".format(i),
                                           self.cnn_hyp_par['n_units_min'],
                                           self.cnn_hyp_par['n_units_max'],
                                           log=True)
            model.add(Dense(num_hidden,
                            activation="relu",
                            activity_regularizer=l1(0.001),
                            name=f'dense_{trial.number}_layer_{i}'))
            dropout_rate = trial.suggest_float("dropout_rate_l{}".format(i),
                                               self.cnn_hyp_par['dropout_min'],
                                               self.cnn_hyp_par['dropout_max'])
            model.add(Dropout(dropout_rate))
        ...
        learning_rate = trial.suggest_float("learning_rate",
                                            self.cnn_hyp_par['learning_rate_min'],
                                            self.cnn_hyp_par['learning_rate_max'],
                                            log=True)
        model.compile(
    ...
    def create_transformer_model(self, input_dim, num_classes, d_model,
                                 num_heads, num_layers, dropout, weight_decay, l1_regularization):
        """"
        This method takes the hyperparameters of the transformer,
        creates a model and then trains and tests the model
        """
    ...
    def transformer_objective(self, trial):
        ...
        d_model = trial.suggest_int('d_model',
                                    *self.transformer_hyp_par['d_model'])
        num_heads = trial.suggest_int('num_heads',
                                      *self.transformer_hyp_par['num_heads'])
        num_layers = trial.suggest_int('num_layers',
                                       *self.transformer_hyp_par['num_layers'])
        dropout = trial.suggest_float('dropout',
                                      *self.transformer_hyp_par['dropout'])
        weight_decay = trial.suggest_float('weight_decay',
                                           *self.transformer_hyp_par['weight_decay'],
                                           log=True)
        l1_regularization = trial.suggest_float('l1_regularization',
                                                *self.transformer_hyp_par['l1_regularization'],
                                                log=True)
        learning_rate = trial.suggest_float('learning_rate',
                                            *self.transformer_hyp_par['learning_rate'],
                                            log=True)
        if d_model % num_heads != 0:
            return 1000.0
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = self.create_transformer_model(self.input_shape[0],
                                              len(np.unique(self.y_train)),
                                              d_model,
                                              num_heads,
                                              num_layers,
                                              dropout, weight_decay,
                                              l1_regularization).to(device)
    ...
    def optimize_transformer(self):
        ...
        study.optimize(lambda trial: self.transformer_objective(trial),
    ...
    def optimize(self, model_type, sampler=None):
        """"
        This method will start the optimization process
        of the hyperparameters for the 4 algorithms
        Args:
            model_type          (string)              : Should be 1 from the
                                                        list "mlp", "xgboost", "cnn", "transformer"
        ...
        if model_type == "mlp":
            study.optimize(self.mlp_objective,
                           n_trials=self.n_trials,
                           gc_after_trial=True)
            print("MLP Best Trial:")
        elif model_type == "xgboost":
            study.optimize(self.xgboost_objective,
                           n_trials=self.n_trials,
                           gc_after_trial=True)
            print("XGBoost Best Trial:")
        elif model_type == "cnn":
            study.optimize(self.cnn_objective,
                           n_trials=self.n_trials,
                           gc_after_trial=True)
